...
from argustrack.app import MainWindow
from argustrack.config import get_config
from argustrack.utils import newIcon
...
def _setup_loguru(logger_level: str) -> None:
    ...
    cache_dir: str
    if os.name == "nt":
        cache_dir = os.path.join(os.environ["LOCALAPPDATA"], "argustrack")
    else:
        cache_dir = os.path.expanduser("~/.cache/argustrack")

    os.makedirs(cache_dir, exist_ok=True)

    log_file = os.path.join(cache_dir, "argustrack.log")
    logger.add(
        log_file,
        colorize=True,
        level="DEBUG",
...
def _handle_exception(exc_type, exc_value, exc_traceback):
    ...
    QtWidgets.QMessageBox.critical(
        None,
        "Error",
        f"An unexpected error occurred. The application will close.<br/><br/>See the log file under your ArgusTrack cache directory for details.<br/><br/>{traceback_html}",  # noqa: E501
    )
...
def main():
    ...
    default_config_file = os.path.join(os.path.expanduser("~"), ".argustrackrc")
    parser.add_argument(
        "--config",
        dest="config",
