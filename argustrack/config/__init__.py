...
def _get_default_config_and_create_argustrackrc():
    config_file = osp.join(here, "default_config.yaml")
    with open(config_file) as f:
        config = yaml.safe_load(f)

    # save default config to ~/.argustrackrc
    user_config_file = osp.join(osp.expanduser("~"), ".argustrackrc")
...
def get_config(config_file_or_yaml=None, config_from_args=None):
    # 1. default config
    config = _get_default_config_and_create_argustrackrc()
